(* ========================================================================= *)
(* Codifica di de Bruijn del lambda calcolo semplice in HOL.                 *)
(* ========================================================================= *)

(* ------------------------------------------------------------------------- *)
(* Riferimenti potenzialmente utili sulla codifica di de Bruijn:
...
(* docker run -it --rm -h hol-light --entrypoint /bin/bash maggesi/hol-light-ckpt:2020-03-06
eval "$(opam config env)"
ocaml -I "$(camlp5 -where)" -init /home/opam/hol-light/hol.ml *)
...
(* ------------------------------------------------------------------------- *)
(* Slide operator (needed for the reindexing operator REINDEX below).        *)
(* ------------------------------------------------------------------------- *)
...
let SLIDE_I = prove
 (`SLIDE I = I`,
  REWRITE_TAC[FUN_EQ_THM] THEN INDUCT_TAC THEN REWRITE_TAC[SLIDE; I_THM]);;

let SLIDE_SLIDE = prove
 (`!f g i. SLIDE f (SLIDE g i) = SLIDE (f o g) i`,
  GEN_TAC THEN GEN_TAC THEN INDUCT_TAC THEN REWRITE_TAC[SLIDE; o_THM]);;
let SLIDE_INJ = prove
 (`!f i j. (!k l. f k = f l ==> k = l) ==> (SLIDE f i = SLIDE f j <=> i = j)`,
  GEN_TAC THEN INDUCT_TAC THEN INDUCT_TAC THEN
...
let REINDEX_EXTENS = prove
 (`!x f g. (!i. i IN FREES x ==> f i = g i) ==> REINDEX f x = REINDEX g x`,
  ...
  [ASM_MESON_TAC[]; FIRST_X_ASSUM MATCH_MP_TAC] THEN INDUCT_TAC THEN
  REWRITE_TAC[SLIDE; SUC_INJ] THEN ASM_SIMP_TAC[]);;
...
let REINDEX_INJ = prove
 (`!x y f. (!i j. f i = f j ==> i = j)
  ...
  DBLAMBDA_INDUCT_TAC THEN DBLAMBDA_INDUCT_TAC THEN
...
let DERIV_SLIDE = prove
 (`!f g i. DERIV g (SLIDE f i) = DERIV (g o f) i`,
  GEN_TAC THEN GEN_TAC THEN INDUCT_TAC THEN REWRITE_TAC[DERIV; SLIDE; o_THM]);;
...
let REINDEX_SLIDE = prove
 (`!g f i. REINDEX (SLIDE g) (DERIV f i) = DERIV (REINDEX g o f) i`,
  GEN_TAC THEN GEN_TAC THEN INDUCT_TAC THEN REWRITE_TAC[DERIV; REINDEX] THEN
...
let REINDEX_EQ_SUBST = prove
 (`!f. REINDEX f = SUBST (REF o f)`,
  ...
  REWRITE_TAC[SUBST_EXTENS] THEN INDUCT_TAC THEN DISCH_TAC THEN
...
let ITER_SLIDE = prove
 (`!f k. ITER k SLIDE f = \i. if i < k then i else k + f (i - k)`,
  GEN_TAC THEN INDUCT_TAC THEN
  ASM_REWRITE_TAC[ITER; ARITH_RULE `~(i < 0)`; FUN_EQ_THM; ADD; SUB_0] THEN
  INDUCT_TAC THEN REWRITE_TAC[SLIDE; ARITH_RULE `0 < SUC k`] THEN
  REWRITE_TAC[LT_SUC; SUB_SUC] THEN COND_CASES_TAC THEN REWRITE_TAC[]);;

let ITER_SLIDE_ADD = prove
 (`!n k i. ITER k SLIDE ((+) n) i = if i < k then i else n + i`,
  REWRITE_TAC[ITER_SLIDE] THEN ARITH_TAC);;
...
let SHIFTI_EQ_REINDEX = prove
 (`!n t k. SHIFTI k n t = REINDEX (ITER k SLIDE ((+) n)) t`,
  ...
  [REWRITE_TAC[ITER_SLIDE_ADD]; GEN_TAC] THEN
  REWRITE_TAC[REINDEX_EXTENS_EQ; ITER]);;

let ITER_DERIV = prove
 (`!f k. ITER k DERIV f =
         \i. if i < k then REF i else REINDEX ((+) k) (f (i - k))`,
  GEN_TAC THEN INDUCT_TAC THEN
  ASM_REWRITE_TAC[ITER; ARITH_RULE `~(i < 0)`; FUN_EQ_THM; SUB_0] THENL
  [SUBGOAL_THEN `(+) 0 = I` (fun th -> REWRITE_TAC[th; REINDEX_I]) THEN
   REWRITE_TAC[FUN_EQ_THM; ADD; I_THM]; ALL_TAC] THEN
  INDUCT_TAC THEN REWRITE_TAC[DERIV; ARITH_RULE `0 < SUC k`] THEN
  REWRITE_TAC[LT_SUC; SUB_SUC] THEN COND_CASES_TAC THEN
  REWRITE_TAC[REINDEX; REINDEX_REINDEX; REINDEX_EXTENS_EQ; o_THM; ADD]);;
...
let SUBST1_EQ_SUBST = prove
 (`!t u. SUBST1 u t = SUBST (PUSHENV u REF) t`,
  ...
  INDUCT_TAC THEN REWRITE_TAC[PUSHENV; NOT_SUC; injectivity "dblambda"] THEN
  ARITH_TAC);;
...
let SUBST1_SUBST1 = prove
 (`!t u w. SUBST1 w (SUBST1 u t) = SUBST1 (SUBST1 w u) (SUBSTI1 1 w t)`,
  ...
  REWRITE_TAC[SUBST1_EQ_SUBST; SUBSTI1_EQ_SUBST; SUBST_SUBST;
              SUBST_EXTENS; ITER_1; o_THM] THEN
  INDUCT_TAC THEN REWRITE_TAC[PUSHENV; DERIV; SUBST] THEN
  POP_ASSUM (K ALL_TAC) THEN DISCH_TAC THEN
  REWRITE_TAC[SUBST_REINDEX] THEN
  STRUCT_CASES_TAC (SPEC `i:num` num_CASES) THEN
  REWRITE_TAC[PUSHENV; NOT_SUC; SUBST; o_THM] THENL
  [MATCH_MP_TAC EQ_SYM THEN REWRITE_TAC[PUSHENV; SUBST_REF_EQ; o_THM];
   REWRITE_TAC[SUBST; injectivity "dblambda"] THEN ARITH_TAC]);;
...
let DBLAMBDA_BETA_RULES, DBLAMBDA_BETA_INDUCT, DBLAMBDA_BETA_CASES =
  ...
  `!x y. DBLAMBDA_BETA (APP (ABS x) y) (SUBST1 y x)`;;

let DBLAMBDA_BETA_INVERSION = prove
 (`!x y z. DBLAMBDA_BETA (APP (ABS x) y) z <=> z = SUBST1 y x`,
...
let DBLAMBDA_BETA_SUBST = prove
 (`!f x y. DBLAMBDA_BETA x y ==> DBLAMBDA_BETA (SUBST f x) (SUBST f y)`,
  ...
  REWRITE_TAC[DBLAMBDA_BETA_INVERSION; SUBST1_EQ_SUBST;
              SUBST_SUBST; SUBST_EXTENS; o_THM] THEN
  INDUCT_TAC THEN REWRITE_TAC[DERIV; SUBST; PUSHENV] THEN
...
let DBLAMBDA_ETA_SUBST = prove
 (`!f x y. DBLAMBDA_ETA x y ==> DBLAMBDA_ETA (SUBST f x) (SUBST f y)`,
  ...
  INDUCT_TAC THEN REWRITE_TAC[DERIV]);;

let DBLAMBDA_ETA_REINDEX = prove
 (`!f x y. DBLAMBDA_ETA x y ==> DBLAMBDA_ETA (REINDEX f x) (REINDEX f y)`,
  REWRITE_TAC[REINDEX_EQ_SUBST] THEN MESON_TAC[DBLAMBDA_ETA_SUBST]);;
...
let BETARED_REINDEX_EXTENS = prove
 (`!x f g. (!i. i IN FREES x ==> f i = g i)
  ...
  [MESON_TAC[BETARED_REFL];
   ...
   FIRST_X_ASSUM MATCH_MP_TAC THEN INDUCT_TAC THEN
...
let BETARED_REINDEX_EXTENS = prove
 (`!x f g. (!i. i IN FREES x ==> f i = g i)
  ...
  INDUCT_TAC THEN REWRITE_TAC[SLIDE] THEN ASM_MESON_TAC[]);;
...
let BETARED_SUBST_EXTENS = prove
 (`!x f g. (!i. i IN FREES x ==> BETARED (f i) (g i))
  ...
  MATCH_MP_TAC BETARED_ABS THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
  INDUCT_TAC THEN REWRITE_TAC[DERIV; BETARED_REFL] THEN
...
let BETAETARED_SUBST_EXTENS = prove
 (`!x f g. (!i. i IN FREES x ==> BETAETARED (f i) (g i))
  ...
  MATCH_MP_TAC BETAETARED_ABS THEN FIRST_X_ASSUM MATCH_MP_TAC THEN
  INDUCT_TAC THEN REWRITE_TAC[DERIV; BETAETARED_REFL] THEN
...
let dblambda_INJECTIVITY = injectivity "dblambda";;
(* |- (!a a'. REF a = REF a' <=> a = a') /\
     (!a0 a1 a0' a1'. APP a0 a1 = APP a0' a1' <=> a0 = a0' /\ a1 = a1') /\
     (!a a'. ABS a = ABS a' <=> a = a') *)

let dblambda_DISTINCTNESS = distinctness "dblambda";;
 (* |- (!a a0' a1'. ~(REF a = APP a0' a1')) /\
     (!a a'. ~(REF a = ABS a')) /\
     (!a0 a1 a'. ~(APP a0 a1 = ABS a')) *)


(* definizione di lambda-termini atomici e composti *)

let DBLAMBDA_ATOMIC_RULES, DBLAMBDA_ATOMIC_INDUCT, DBLAMBDA_ATOMIC_CASES =
  new_inductive_definition
  `!i. DBLAMBDA_ATOMIC (REF i)`;;

let DBLAMBDA_COMPOUND_RULES, DBLAMBDA_COMPOUND_INDUCT, DBLAMBDA_COMPOUND_CASES =
  new_inductive_definition
  `(!x. DBLAMBDA_COMPOUND (ABS x)) /\
   (!x y. DBLAMBDA_COMPOUND (APP x y))`;;


g `(!i. DBLAMBDA_ATOMIC (REF i)) /\
   (!x. ~ (DBLAMBDA_ATOMIC (ABS x))) /\
   (!x y. ~ (DBLAMBDA_ATOMIC (APP x y)))`;;
e (REWRITE_TAC [DBLAMBDA_ATOMIC_RULES]);;
e (ONCE_REWRITE_TAC [DBLAMBDA_ATOMIC_CASES]);;
e (REWRITE_TAC [dblambda_DISTINCTNESS]);;
let DBLAMBDA_ATOMIC_CLAUSES = top_thm();;

g `(!x. DBLAMBDA_COMPOUND (ABS x)) /\ 
    (!x y. DBLAMBDA_COMPOUND (APP x y)) /\ 
    (!i. ~ DBLAMBDA_COMPOUND (REF i))`;;
e (REWRITE_TAC [DBLAMBDA_COMPOUND_RULES]);;
e (ONCE_REWRITE_TAC [DBLAMBDA_COMPOUND_CASES]);;
e (REWRITE_TAC [dblambda_DISTINCTNESS]);;
let DBLAMBDA_COMPOUND_CLAUSES = top_thm ();;

g `!x. DBLAMBDA_ATOMIC x ==> ~ (DBLAMBDA_COMPOUND x)`;;
e DBLAMBDA_INDUCT_TAC;;
e (REWRITE_TAC [DBLAMBDA_ATOMIC_RULES; DBLAMBDA_COMPOUND_CLAUSES]);;
e (MESON_TAC [DBLAMBDA_ATOMIC_CLAUSES]);;
e (MESON_TAC [DBLAMBDA_ATOMIC_CLAUSES]);;
let ATOMIC_LEMMA = top_thm();;

g `!x. DBLAMBDA_COMPOUND x ==> ~ (DBLAMBDA_ATOMIC x)`;;
e DBLAMBDA_INDUCT_TAC;;
e (MESON_TAC [DBLAMBDA_COMPOUND_CLAUSES]);;
e (MESON_TAC [DBLAMBDA_ATOMIC_CLAUSES]);;
e (MESON_TAC [DBLAMBDA_ATOMIC_CLAUSES]);;
let COMPOUND_LEMMA = top_thm();;




(* Definizione della relazione di beta-riduzione parallela *)

let MINARI_RULES, MINARI_INDUCT, MINARI_CASES = 
new_inductive_definition
`(!i. MINARI_BETARED (REF i) (REF i)) /\
(!x y. MINARI_BETARED x y ==> MINARI_BETARED (ABS x) (ABS y)) /\
(!x1 y1 x2 y2. MINARI_BETARED x1 y1 /\ MINARI_BETARED x2 y2 ==> MINARI_BETARED (APP x1 x2) (APP y1 y2)) /\
(!x1 y1 x2 y2. MINARI_BETARED x1 y1 /\ MINARI_BETARED x2 y2 ==> MINARI_BETARED (APP (ABS x1) x2) (SUBST1 y2 y1))`;;

g `!i. MINARI_BETARED (REF i) (REF i)`;;
e (REWRITE_TAC [MINARI_RULES]);;
let MINARI_RHO' = top_thm ();;

g `!x y. MINARI_BETARED x y ==> MINARI_BETARED (ABS x) (ABS y)`;;
e (REWRITE_TAC [MINARI_RULES]);;
let MINARI_CSI = top_thm ();;

g `!x1 y1 x2 y2. MINARI_BETARED x1 y1 /\ MINARI_BETARED x2 y2 ==> MINARI_BETARED (APP x1 x2) (APP y1 y2)`;;
e (REWRITE_TAC [MINARI_RULES]);;
let MINARI_APP = top_thm ();;

g `!x1 y1 x2 y2. MINARI_BETARED x1 y1 /\ MINARI_BETARED x2 y2 ==> MINARI_BETARED (APP (ABS x1) x2) (SUBST1 y2 y1)`;;
e (REWRITE_TAC [MINARI_RULES]);;
let MINARI_BETA = top_thm ();;

g `!x. MINARI_BETARED x x`;;
e DBLAMBDA_INDUCT_TAC;;
e (REWRITE_TAC [MINARI_RHO']);;
e (ASM_MESON_TAC [MINARI_APP]);;
e (ASM_MESON_TAC [MINARI_CSI]);;
let MINARI_RHO = top_thm();;

g `!x. DBLAMBDA_ATOMIC x ==> MINARI_BETARED x x`;;
e (REWRITE_TAC [DBLAMBDA_ATOMIC_CASES]);;
e (REPEAT STRIP_TAC);;
e (ASM_REWRITE_TAC [MINARI_RULES]);;
let LEMMA_AT = top_thm();;


(* Lemmi di Inversione *)

g `!x i. MINARI_BETARED (REF i) x <=> x = REF i`;;
e (REPEAT STRIP_TAC);;
e (GEN_REWRITE_TAC LAND_CONV [MINARI_CASES]);;
e (REWRITE_TAC [dblambda_DISTINCTNESS; dblambda_INJECTIVITY]);;
e (MESON_TAC []);;
let RHO_INVERSION = top_thm();;

g `!x1 y. MINARI_BETARED (ABS x1) y <=> ?y1. y = (ABS y1) /\ MINARI_BETARED x1 y1`;;
e (REPEAT STRIP_TAC);; 
e (GEN_REWRITE_TAC LAND_CONV [MINARI_CASES]);;
e (REWRITE_TAC [dblambda_DISTINCTNESS]);;
e (REWRITE_TAC [dblambda_INJECTIVITY]);;
e (MESON_TAC []);;
let CSI_INVERSION = top_thm();;

g `!x1 x2 y.
     MINARI_BETARED (APP x1 x2) y <=> (?y1 y2.y = (APP y1 y2) /\
                                                  MINARI_BETARED x1 y1 /\
                                                  MINARI_BETARED x2 y2) \/
                                      (?x3 x4 y2 y3. x1 = (ABS x3) /\
                                                     y = (SUBST1 y3 x4) /\
                                                     MINARI_BETARED x3 x4 /\
                                                     MINARI_BETARED x2 y3)`;;
e (REPEAT STRIP_TAC);;
e (GEN_REWRITE_TAC LAND_CONV [MINARI_CASES]);;
e (REWRITE_TAC [dblambda_DISTINCTNESS]);;
e (REWRITE_TAC [dblambda_INJECTIVITY]);;
e (MESON_TAC []);;
let APP_BETA_INVERSION = top_thm();;


(* Lemma di sostituzione *)

(* g `!z x y. MINARI_BETARED x y ==> MINARI_BETARED (SUBST1 x z) (SUBST1 y z)`;;
e GEN_TAC;;
e DBLAMBDA_INDUCT_TAC;;
e (REPEAT GEN_TAC);;
e DISCH_TAC;;
...
g `!t s r q. MINARI_BETARED t s /\ MINARI_BETARED r q ==> MINARI_BETARED (SUBST1 r t) (SUBST1 q s)`;;
e (GEN_TAC THEN GEN_TAC THEN GEN_TAC THEN GEN_TAC);;
e (GEN_REWRITE_TAC (LAND_CONV o LAND_CONV) [MINARI_CASES]);;
e (REPEAT STRIP_TAC);;
e (ASM_REWRITE_TAC [SUBST1; SUBSTI1]);; *)



(* DIAMOND_PROPERTY *)

let DIAMOND_PROPERTY = new_definition 
  `DIAMOND_PROPERTY (R: dblambda->dblambda->bool) <=>
   (!x y z. R x y /\ R x z ==> ?u. R y u /\ R z u)`;;


(* CHURCH_ROSSER *)

g `DIAMOND_PROPERTY MINARI_BETARED`;;
e (REWRITE_TAC [DIAMOND_PROPERTY]);;
e DBLAMBDA_INDUCT_TAC;;
(* Ref *)
e (REWRITE_TAC [RHO_INVERSION]);;
e (MESON_TAC [MINARI_RHO']);;
r 1;;
(* Abs *)
e (REWRITE_TAC [CSI_INVERSION]);;
e (ASM_MESON_TAC [MINARI_CSI]);;
e (ONCE_REWRITE_TAC [APP_BETA_INVERSION]);;
e (REPEAT STRIP_TAC);;
e (ASM_MESON_TAC [MINARI_APP]);;

let CHURCH_ROSSER = top_thm();;




(* let LSUBTERM_RULES, LSUBTERM_INDUCT, LSUBTERM_CASES = new_inductive_definition
`(!x. SUBTERM x x) /\
(!x y. SUBTERM x (APP x y)) /\
(!x y. SUBTERM y (APP x y)) /\
(!x. SUBTERM x (ABS x))`;;

let LAMBDA_NORMAL_FORM = new_definition 
`(LAMBDA_NORMAL_FORM x) <=> ~ (? y z. (SUBTERM (APP (ABS y) z) x))`;;

let LAMBDA_IND_NORMAL_FORM_RULES, LAMBDA_IND_NORMAL_FORM_INDUCT, LAMBDA_IND_NORMAL_FORM_CASES = new_inductive_definition
`(!i. LAMBDA_IND_NORMAL_FORM (REF i)) /\
(!x. LAMBDA_IND_NORMAL_FORM x ==> LAMBDA_IND_NORMAL_FORM (ABS x)) /\
(!x y. LAMBDA_IND_NORMAL_FORM x /\ LAMBDA_IND_NORMAL_FORM y /\ ~ (? z. x = (ABS z)) ==> LAMBDA_IND_NORMAL_FORM (APP x y))`;;

g `!x. LAMBDA_NORMAL_FORM x ==> LAMBDA_IND_NORMAL_FORM x`;;
e DBLAMBDA_INDUCT_TAC;;
e (MESON_TAC [LAMBDA_IND_NORMAL_FORM_RULES]);;
r 1;;
e (ASM_MESON_TAC [LAMBDA_NORMAL_FORM; LSUBTERM_RULES; LAMBDA_IND_NORMAL_FORM_RULES]);; *)
