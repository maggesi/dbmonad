...
let INITIAL_MORPHISM_MODULARITY = prove
 (`!sig1 sig2 s ref fn subst h:rterm->A.
     h IN MODEL_MOR (sig1 UNION sig2)
           (WELLFORMED (sig1 UNION sig2),TMREF,FN,TMSUBST)
           (s,ref,fn,subst) /\
     (!x. x IN WELLFORMED sig1 ==> h x = INITIAL_MORPHISM (ref,fn) x) /\
     (!x. x IN WELLFORMED sig2 ==> h x = INITIAL_MORPHISM (ref,fn) x)
     ==>
     (!x. x IN WELLFORMED (sig1 UNION sig2)
          ==> h x = INITIAL_MORPHISM (ref,fn) x)`,
  REPEAT GEN_TAC THEN INTRO_TAC "h h1 h2" THEN
  MATCH_MP_TAC WELLFORMED_INDUCT THEN
  REWRITE_TAC[INITIAL_MORPHISM_CLAUSES] THEN CONJ_TAC THENL
  [HYP_TAC "h" (REWRITE_RULE[MODEL_MOR; IN_ELIM_THM]) THEN
   ASM_REWRITE_TAC[];
   ALL_TAC] THEN
  HYP_TAC "h -> _ model h h1 h2 subst"
    (REWRITE_RULE[MODEL_MOR; IN_ELIM_THM]) THEN
  REPEAT GEN_TAC THEN REWRITE_TAC[IN_UNION] THEN INTRO_TAC "(c|c) hx" THENL
  [REMOVE_THEN "h2" CASE_REWRITE_TAC THEN
   SUBGOAL_THEN `c,MAP FST (args:(num#rterm)list) IN sig1 UNION sig2:signature`
     (fun th -> REWRITE_TAC[th]) THENL
   [REMOVE_THEN "c" MP_TAC THEN SET_TAC[]; ALL_TAC] THEN
   AP_TERM_TAC THEN  MATCH_MP_TAC MAP_EQ THEN
   REWRITE_TAC[GSYM ALL_MEM; FORALL_PAIR_THM; PAIR_EQ] THEN
   HYP MESON_TAC "hx" []
  ;

   REMOVE_THEN "h1" CASE_REWRITE_TAC THEN
   SUBGOAL_THEN `c,MAP FST (args:(num#rterm)list) IN sig1 UNION sig2:signature`
     (fun th -> REWRITE_TAC[th]) THENL
   [REMOVE_THEN "c" MP_TAC THEN SET_TAC[]; ALL_TAC] THEN
   AP_TERM_TAC THEN  MATCH_MP_TAC MAP_EQ THEN
