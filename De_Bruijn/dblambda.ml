...
let ITER_SLIDE = prove
 (`!f k. ITER k SLIDE f = \i. if i < k then i else k + f (i - k)`,
  ...
  CASES_GEN_TAC THEN REWRITE_TAC[SLIDE; ARITH_RULE `0 < SUC k`] THEN
...
let ITER_DERIV = prove
 (`!f k. ITER k DERIV f =
  ...
  CASES_GEN_TAC THEN REWRITE_TAC[DERIV; ARITH_RULE `0 < SUC k`] THEN
...
let DBLAMBDA_BETA_SUBST = prove
 (`!f x y. DBLAMBDA_BETA x y ==> DBLAMBDA_BETA (SUBST f x) (SUBST f y)`,
  ...
  CASES_GEN_TAC THEN REWRITE_TAC[DERIV; SUBST; PUSHENV] THEN
